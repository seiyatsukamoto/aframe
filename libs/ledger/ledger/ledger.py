...
class Ledger:
    ...
    def __getitem__(self, *args, **kwargs):
        ...
        for key, attr in self.__dataclass_fields__.items():
            value = getattr(self, key)
            if type(value) == dict: #To do: No try except and no metadata check
            ...
            for source in tqdm(_iter_open(files, "r", clean=clean)):
                ...
                for key, attr in cls.__dataclass_fields__.items():
                    ...
                    if attr.metadata["kind"] == "metadata":
                        ...
                        if attr.type == dict: 
                            if not key in source[group_name]: #The dict is kept to be backwards compatible. It will be silent if there is dict in source
                                continue
