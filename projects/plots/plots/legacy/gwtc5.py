...
def get_injection_data(
    ...
    url = (
        "https://zenodo.org/records/19500064/files/"
        "samples-rpo4ab-1366933504-55469568-clipped.hdf"
    )
    logging.info(
    ...
    with h5py.File(injection_file, "r") as f:
        ...
        injection_params["m1"] = f["events"]["mass1_source"][:]
        injection_params["m2"] = f["events"]["mass2_source"][:]
        injection_params["s1x"] = f["events"]["spin1x"][:]
        injection_params["s1y"] = f["events"]["spin1y"][:]
        injection_params["s1z"] = f["events"]["spin1z"][:]
        injection_params["s2x"] = f["events"]["spin2x"][:]
        injection_params["s2y"] = f["events"]["spin2y"][:]
        injection_params["s2z"] = f["events"]["spin2z"][:]
        injection_params["z"] = f["events"]["z"][:]
    
        ln_spin1 = (f["events"]["lnpdraw_spin1_magnitude"] 
                    + f["events"]["lnpdraw_spin1_polar_angle"] 
                    + f["events"]["lnpdraw_spin1_azimuthal_angle"] 
                    - 2*np.log(f["events"]["spin1_magnitude"]) 
                    - np.log(np.sin(f["events"]["spin1_polar_angle"])))
    
        ln_spin2 = (f["events"]["lnpdraw_spin2_magnitude"] 
                    + f["events"]["lnpdraw_spin2_polar_angle"] 
                    + f["events"]["lnpdraw_spin2_azimuthal_angle"]
                    - 2*np.log(f["events"]["spin2_magnitude"]) 
                    - np.log(np.sin(f["events"]["spin2_polar_angle"])))
        
        p_draw = (f["events"]["lnpdraw_mass1_source"] 
                  + f["events"]["lnpdraw_mass2_source_GIVEN_mass1_source"] 
                  + f["events"]["lnpdraw_z"] 
                  + ln_spin1 + ln_spin2)
        p_draw = np.exp(p_draw)
        
        det_stat = {}
        for p in pipelines:
            det_stat[p] = f["events"][f"{p}_{detection_criterion}"][:]
...
def main(
    ...
    known_pipelines = ["cwb-bbh", "gstlal", "mbta", "pycbc"]
