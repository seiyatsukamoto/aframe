...
import ml4gw
class SpectrogramAutoencoderAframeDataset(SupervisedAframeDataset):
    def __init__(
        self, 
        q: float, 
    ...
    def train_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        return ZippedDataset(dataloader, bbh_waveform_dataset, bns_waveform_dataset)
    ...
    def on_before_batch_transfer(self, batch, _):
        if self.trainer.training and self.waveforms_from_disk:
            X, bbh_waveforms, bns_waveforms = batch
    ...
    def on_after_batch_transfer(self, batch, _):
        if self.trainer.training:
            ...
            [background, _, timeslide_idx], [bbh_signals], [bns_signals] = batch
            ...
            X_bg, X_fg, X_fg_target = self.build_val_batches(background, signals)
            batch = (X_bg, X_fg, X_fg_target)
        return batch
    ...
    def inject(self, X, waveforms=None):
        ...
        X_target = self.qtransform(X_target)
        mins = torch.amin(X_target, dim = [2, 3], keepdim=True)
        maxes = torch.amax(X_target, dim = [2, 3], keepdim=True)
        X_target = (X_target-mins)/(maxes-mins).clamp_min(1e-8)
        return X, X_target
