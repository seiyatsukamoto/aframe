...
class ZippedTrainDataset(torch.utils.data.IterableDataset):
    ...
    def __iter__(self):
        for X, bbh_waveforms, bns_waveforms in zip(*self.datasets, strict=False):
...
class ZippedValDataset(torch.utils.data.IterableDataset):
    ...
    def __iter__(self):
        for [background, b, timeslide_idx], [bbh_waveforms], [bns_waveforms] in zip(*self.datasets, strict=False):
            waveforms = torch.concat((bbh_waveforms, bns_waveforms), dim = 0)
