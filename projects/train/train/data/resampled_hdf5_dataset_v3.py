...
class ResampledHdf5TimeSeriesDataset_v3(torch.utils.data.IterableDataset):
    """
    ...
    def sample_batch(self) -> WaveformTensor:
        ...
        x = np.zeros((self.batch_size, len(self.channels), self.kernel_size//self.downsample_rate))
        ...
        for i, (fname, count) in enumerate(zip(unique_fnames, counts)):
            ...
            with h5py.File(fname, "r") as f:
                for b, c, i in zip(batch_indices, channel_indices, idx):
                    x[b, c] = f[self.channels[c]][i:i + self.kernel_size:self.downsample_rate]
        return x
