...
from aframe.tasks.infer.mc_base import InferBase, InferParameters
...
class Infer(AframeSingularityTask):
    ...
    def run(self):
        ...
        from ledger.mc_events import EventSet, RecoveredInjectionSet
        ...
        for i in range(10):
            ...
            EventSet.aggregate(
                back_files,
                self.background_output[i],
                clean=False,
                length=background_length[i],
                i = i,
            )
            logging.info("Aggregating foreground files")
            RecoveredInjectionSet.aggregate(
                self.foreground_files[foreground_mask[i]],
                self.foreground_output[i],
                clean=False,
                length=foreground_length[i],
                i = i,
            )
            if len(zero_lag_files) > 0:
                ...
                EventSet.aggregate(
                    zero_lag_files,
                    self.zero_lag_output[i],
                    clean=False,
                    length=zero_lag_length[i],
                    i = i,
                )
            if self.return_timeseries: #dont return timeseries 
        ...
        # TODO: any benefit to sorting foreground for SV calculation?
            if len(back_files) > 0:
                background = EventSet.read(self.background_output[i])
                background = background.sort_by("detection_statistic")
                background.write(self.background_output[i])

        #if self.remove_tmpdir: #for now 
